...
export function HashChainVerifier({ blocks, title = "Hash Chain Verification", hashDisplayLength = 8, onBlockClick, showSummary = true, className, }) {
    ...
    const summaryBadge = showSummary ? (_jsx("span", { className: `cc-hash-chain-verifier__summary cc-hash-chain-verifier__summary--${allVerified ? "ok" : "broken"}`, style: {
            display: "inline-flex",
            alignItems: "center",
            ...
            : `${brokenCount} broken link${brokenCount !== 1 ? "s" : ""}` })) : undefined;
    return (_jsx(Card, { title: title, actions: summaryBadge, padded: true, className: classes, role: "region", "aria-label": title, children: _jsx("div", { className: "cc-hash-chain-verifier__chain", style: {
                display: "flex",
