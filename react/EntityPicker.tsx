...
export function EntityPicker<E>(props: EntityPickerProps<E>): React.ReactElement {
  const {
    search,
    renderOption,
    value,
  ...
  // ── Results ─────────────────────────────────────────────────────────────────
  const [results, setResults] = React.useState<E[]>([]);
  ...
  // ── Selection logic ──────────────────────────────────────────────────────────
  const selectedItems: E[] = React.useMemo(() => {
  ...
  return (
    <div className="cc-entity-picker">
      {/* Selected chips (multi-mode) */}
      {multi && selectedItems.length > 0 ? (
        <div className="cc-entity-picker__chips" role="list" aria-label="Selected items">
          {selectedItems.map((item, i) => (
            <span
              key={i}
              role="listitem"
              className="cc-entity-picker__chip"
            >
              <span className="cc-entity-picker__chip-label">{renderOption(item)}</span>
              <button
                type="button"
                className="cc-entity-picker__chip-remove"
                aria-label="Remove"
      ...
      {/* Single-mode selection display */}
      {!multi && value !== null && !Array.isArray(value) ? (
        <div className="cc-entity-picker__single-value">
          <span className="cc-entity-picker__single-label">{renderOption(value as E)}</span>
          <button
            type="button"
            className="cc-entity-picker__clear"
      ...
      {(multi || value === null || Array.isArray(value)) ? (
        ...
        >
          <input
            ref={inputRef}
            type="text"
            className="cc-entity-picker__input"
            value={query}
          ...
          {open ? (
            <ul
              ...
              className="cc-entity-picker__listbox"
              style={{
                position: 'absolute',
                zIndex: 50,
            ...
            >
              {loading ? (
                <li className="cc-entity-picker__loading" aria-live="polite">
                  Searching…
                </li>
              ) : results.length === 0 ? (
                <li className="cc-entity-picker__empty">{emptyText}</li>
              ) : (
                results.map((item, i) => (
                  <li
                    key={i}
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-sm)',
                      background:
                        i === activeIndex ? 'var(--surface-1)' : undefined,
                    }}
              ...
              {showCreateButton ? (
                <li
                  role="option"
                  aria-selected={false}
                  className="cc-entity-picker__create-option"
                  style={{
                    padding: 'var(--space-1) var(--space-3)',
                    cursor: 'pointer',
                    borderRadius: 'var(--radius-sm)',
      ...
      {allowCreate && createSchema && createOpen ? (
        <Overlay
          placement="modal"
          open={createOpen}
        ...
        >
          <EntityForm
            ...
            initialValues={
              Object.fromEntries(
                Object.keys(createSchema._zodSchema.shape).map((k) => [k, '']),
              ) as any
            }
            onSubmit={async (values) => {
              await handleCreateSubmit(values as Partial<E>);
            }}
            submitLabel="Create"
          >
            {(form) =>
              Object.keys(createSchema._zodSchema.shape).map((fieldName) => (
                <TextField key={fieldName} name={fieldName} form={form as any} />
              ))
            }
          </EntityForm>
        </Overlay>
